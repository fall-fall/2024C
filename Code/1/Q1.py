# -*- coding: utf-8 -*-
# 文件名: solve_q1_with_sensitivity_analysis.py
...
def run_sensitivity_analysis(base_params, target_crop, param_to_vary, variation_range, case='discount'):
    ...
    Args:
        base_params (dict): 原始的基础参数字典。
        target_crop (str): 要分析的目标作物名称。
        param_to_vary (str): 要变化的参数名称 ('P_price' 或 'P_cost')。
        variation_range (list or np.array): 变化百分比的范围, e.g., np.arange(-0.2, 0.21, 0.05)。
        case (str): 问题一的情形, 'waste' 或 'discount'。

    Returns:
        tuple: (variations_list, profits_list)
    """
...
def plot_sensitivity_gradient(variations, profits, title, xlabel):
    ...
    # 保存图像
    output_filename = "sensitivity_analysis_plot.png"
...
if __name__ == '__main__':
    ...
    if params:
        ...
        # 3. 运行灵敏度分析 (以情况二'discount'为例，因为它更符合经济实际)
        variations, profits = run_sensitivity_analysis(
            base_params=params,
            target_crop=HIGH_VALUE_CROP,
            param_to_vary='P_price', # 分析销售价格
        ...
        plot_sensitivity_gradient(
            variations, 
            profits,
            title=f'图1：{HIGH_VALUE_CROP}销售价格波动对七年总利润的影响分析',
            xlabel=f'{HIGH_VALUE_CROP} 销售价格基准值波动'
        )